...
def safe_time(val):
    if val is None:
        return None
    return str(val)   # converts timedelta / time → "HH:MM:SS"
...
def register_user():
    ...
    try:
        ...
        with conn.cursor(dictionary=True) as cur:
            ...
            cur.execute("SELECT LAST_INSERT_ID() AS user_id")
...
def get_user():
    ...
    try:
        ...
        with conn.cursor(dictionary=True) as cur:
            cur.execute("""
                SELECT user_id, first_name, middle_name, last_name, user_name, age
                FROM users
                WHERE user_name = %s
            """, (user_name,))
            user = cur.fetchone()
